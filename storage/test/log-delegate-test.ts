...
describe("Test static proxy calls", function () {
    ...
    it("Should not delegate set call through proxy contract", async function () {
        let proxyContract = new ethers.Contract(proxy.address, abi, deployer);
        try {
            // try to set the value which should fail
            await proxyContract.setValue(2);
        } catch (error) {
            // ignore error
        }
        // validate that the setValue did not succeed and the contract variable still has its old value
        expect(await proxyContract.getValue()).to.equal(37)
    });

    it("Should not delegate set call through calling contract", async function () {
        let callingContract = new ethers.Contract(caller.address, abi, deployer);
        try {
            await callingContract.setValue(2);
        } catch (error) {
            // ignore error
        }
        expect(await callingContract.getValue()).to.equal(37);
    });
