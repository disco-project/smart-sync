...
describe("Test scaling of contract", async function () {
    ...
    //     let txResponse = await proxyContract.updateStorage(rlpProof);
    //     let receipt = await txResponse.wait();
    //     console.log("Gas used for updating 10 values in map with 10 values: ", receipt.gasUsed.toNumber());
    ...
    it("Contract with map containing 50 values, update 1 value", async function() {
        ...
        let proof = new GetProof(await provider.send("eth_getProof", [srcContract.address, keys]));
        encodedProof = await proof.encoded(latestBlock.stateRoot);
        console.log(encodedProof.toString('hex'));
        ...
        let txResponse = await proxyContract.updateStorage(rlpProof);
        let receipt = await txResponse.wait();
        console.log("Gas used for updating 1 value in map with 50 values: ", receipt.gasUsed.toNumber());
