...
describe("update-multiple-values-random-with-map-sizes-1-1000", async function () {
    ...
    before(() => {
        ...
        csvManager = new CSVManager<CSVDataTemplateMultipleValues>(`measurements-multiple-values-random-with-map-sizes-1-to-1000.csv`);
        provider = new ethers.providers.JsonRpcProvider({ url: httpConfig.url, timeout: 3600000 });
    ...
    it("Contract with map containing 10 values, update multiple values random per iteration", async function() {
        const map_size = 10;
        ...
        for (let i = 0; i < map_size; i++) {
            ...
            const result = await chainProxy.changeRandomValues(value_count, MAX_VALUE);
            ...
            logger.info(`value count: ${value_count}, gas used: `, migrationResult.receipt.gasUsed.toNumber());
    ...
    it("Contract with map containing 100 values, update multiple values random per iteration", async function() {
        const map_size = 100;
        const initialization = await chainProxy.initializeProxyContract(map_size, MAX_VALUE);
        expect(initialization.migrationState).to.be.true;
        currBlockNr = await provider.getBlockNumber() + 1;
    ...
    it("Contract with map containing 1000 values, update multiple values random per iteration", async function() {
        const map_size = 1000;
        const initialization = await chainProxy.initializeProxyContract(map_size, MAX_VALUE);
        expect(initialization.migrationState).to.be.true;
        currBlockNr = await provider.getBlockNumber() + 1;
