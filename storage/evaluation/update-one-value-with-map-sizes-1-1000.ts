...
describe("Test scaling of contract", async function () {
    ...
    after(async () => {
        return new Promise(resolve => {
            ...
            const writeStream = fs.createWriteStream(`./evaluation/csv-files/${time}_measurements-one-value-with-map-sizes-1-to-1000.csv`);
    ...
    it("Contract with map containing 1 value, update 1 value", async function () {
        ...
        if (counter != 0) await proxyContract.addStorage(proxykeys, proxyValues, { gasLimit: httpConfig.gas });
        logger.debug('done.');
    ...
    it("Contract with map containing 10 values, update 1 value per iteration", async function() {
        ...
        const migrationValidated = await relayContract.getMigrationState(proxyContract.address);
        expect(migrationValidated).to.be.true;
        logger.debug(`correct storage root: ${proof.storageHash}`)
        ...
        for (let i = 0; i < map_size; i++) {
            ...
            latestBlock = await provider.send('eth_getBlockByNumber', ["latest", false]);

            // create a proof of the source contract's storage for all the changed keys
            let nextProof = new GetProof(await provider.send("eth_getProof", [srcContract.address, changedKeys]));

            // get depth of value
            let value_mpt_depth = 0;
            nextProof.storageProof.forEach((storageProof) => {
                if (value_mpt_depth < storageProof.proof.length) value_mpt_depth = storageProof.proof.length;
            });

            // compute the optimized storage proof
            const rlpOptimized = nextProof.optimizedStorageProof();

            // ensure that the old contract state equals the last synced storage hash
            try {
                const validated = await proxyContract.verifyOldContractStateProof(rlpOptimized);
                logger.debug(validated.toString('hex'));
                expect(validated).to.be.true;
            } catch(e) {
                logger.error('something went wrong');
                const regexr = new RegExp(/Reverted 0x(.*)/);
            ...
            const rlpProof = await nextProof.optimizedProof(latestBlock.stateRoot);
            await relayContract.updateBlock(latestBlock.stateRoot, latestBlock.number);
            ...
            logger.info(`Gas used for updating value at index ${i} in map with ${map_size} values: `, receipt.gasUsed.toNumber());
