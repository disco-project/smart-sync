...
async function processParameters(srcAddress: string, srcBlock?: string | number, targetAddress?: string, targetBlock?: string | number): Promise<ProcessedParameters> {
    ...
    realSrcBlock = await toBlockNumber(realSrcBlock);
    realTargetBlock = await toBlockNumber(realTargetBlock);
...
class DiffHandler {
    ...
    async getDiffFromStorage(srcAddress: string, targetAddress?: string, srcBlock?: string | number, targetBlock?: string | number): Promise<StorageDiff> {
        const processedParameters: ProcessedParameters = await processParameters(srcAddress, srcBlock, targetAddress, targetBlock);
    ...
    async getDiffFromSrcContractTxs(srcAddress: string, latestSrcBlock?: string | number, earliestSrcBlock?: string | number): Promise<StorageDiff> {
        const processedParameters: ProcessedParameters = await processParameters(srcAddress, latestSrcBlock, srcAddress, earliestSrcBlock);
